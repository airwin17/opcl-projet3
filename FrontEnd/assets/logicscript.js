...
async function trytocreateworks(){
    ...
    var formdata=new FormData(neworkform);
...
try{
    ...
    const response=await fetch("http://localhost:5678/api/works",
    {
        ...
        headers:{
            ...
            'Cntent-Type':'multipart/form-data'
        },
        body:formdata
    });
...
//check form validity
function checkerworks(){
    console.log(get("input#work-ttl-input").checkValidity());
    console.log(get("#inputfile").files.item(0).size);
    if(get("#inputfile").files.item(0).size <= 4097152){
        reader.readAsDataURL(get("#inputfile").files[0]);
